                ...
                blocks.append({
                    ...
                    "extra_attrs": {
                        "partition_icon": (
                            ...
                            "{%- set is_hidden = p.is_hidden | default(false) -%}"
                            "{%- set is_encrypted = p.encryption.encrypted | default(false) if p.encryption is defined else false -%}"
                            "{%- set enc_unlocked = p.encryption.unlocked | default(false) if p.encryption is defined else false -%}"
                        ...
                        "partition_secondary": (
                            ...
                            "{%- set vol_guid = (p.volume_guid | default('')) | replace('{', '') | replace('}', '') -%}"
                            "{%- set is_encrypted = p.encryption.encrypted | default(false) if p.encryption is defined else false -%}"
                            "{%- set enc_scheme = p.encryption.scheme | default('') if p.encryption is defined else '' -%}"
                            "{%- set enc_unlocked = p.encryption.unlocked | default(false) if p.encryption is defined else false -%}"
                            "{%- set flags = [] -%}"
