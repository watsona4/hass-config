...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    def _persist_refresh_token(new_refresh_token: str) -> None:
        """Save a rotated refresh_token so the next restart doesn't reuse a dead one."""
        hass.config_entries.async_update_entry(
            entry,
            data={
                **entry.data,
                CONF_REFRESH_TOKEN: new_refresh_token,
                CONF_TOKEN: {"refresh_token": new_refresh_token},
            },
        )

    session = async_get_clientsession(hass)
    client = ResideoApiClient(session, refresh_token, on_token_refresh=_persist_refresh_token)
