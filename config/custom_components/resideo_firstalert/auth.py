    ...
    async def _step3_submit_credentials(
        ...
        async with session.post(
            ...
            if resp.status != 200:
                ...
                # Log the actual response body -- without this, a non-200 that
                # ISN'T a plain wrong-password rejection (MFA/step-up challenge,
                # CAPTCHA, anomaly-detection block after a recent password
                # change, rate limiting, etc.) is indistinguishable from a wrong
                # password in the logs, both just say "Login failed with status
                # 401". Diagnosing 2026-08-15: repeated 401s here despite a
                # confirmed-correct, just-changed password.
                _LOGGER.error(
                    "Step 3 login failed with status %s, response body: %s",
                    resp.status, text[:2000],
                )
